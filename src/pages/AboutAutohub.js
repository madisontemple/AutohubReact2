...
<button onClick={() => setCount(prevCount => prevCount + 1)}>+</button>
  ...
  function Tr({ title, value }) {
    return (
      <tr style={{}}>
        ...
        <td style={{ borderBottom: "1px solid #ddd", paddingLeft: "80px" }}>
          {value}
        </td>
  ...
  const HeaderLists = [
    { list: "단지안내" },
    { url:"./IntroAutohub/Floor",list: "층별안내" },
    { list: "고객센터" },
    { list: "FAQ" },
    { list: "오시는 길" },
  ];
  const BusinessList = [
    { title:<p style={{paddingRight:"10px"}}>Our Business</p>,
      topic: <p style={{paddingRight:"350px"}}>단일규모 세계 최대 자동차 복합단지입니다.</p>,
      content: (
        <p style={{paddingRight:"340px"}}>
          오토허브는 단일규모 세계 최대 자동차 복합단지로 현재 약 8,000 여대의
          실내 전시가 가능하며,
          <br />
          향후 2차 증축사업과 야외 전시장 확장을 통해 18,000대까지 전시가 가능한
          대규모 복합단지입니다.
        </p>
      ),
    },
    {
      topic: (
        <p>대규모 정비단지 및 성능점검장을 통해 판매를 진행하고 있습니다.</p>
      ),
      content: <p>또한 대규모 정비단지 및 성능점검장을 통해</p>,
    },
    {
      topic: "대규모 정비단지 및 성능점검장을 통해 판매를 진행하고 있습니다.",
      content: (
        <p>
          또한 대규모 정비단지 및 성능점검장을 통해
          <br />
          <span style={{}}>-</span> 직영 성능점검장
          <br />
          - 1급 종합정보운영
          <br />- 연장보증보험(Extended Warranty)판매를 진행하고 있습니다.
        </p>
      ),
    },

    {
      topic: "전시장 관제, 실매물 광고 관리하고 있습니다.",
      content: (
        <p>
          전시장 관제, 실매물 광고 관리를 통해 내부 자체적인 전시차량 입출과
          전산 시스템으로
          <br />
          온라인과 오프라인이 100% 연동되어 허위매물없이 실매물을 관리하고
          있습니다.
        </p>
      ),
    },
    {
      topic: "원스톱 자동차 복합 문화공간이 준비되어 있습니다.",
      content: (
        <p>
          원스톱 자동차 복합 문화공간은 자동차 뿐만 아니라,
          <br />
          쇼핑, 스포츠, 문화생활, 외식 등 온 가족이 즐길 수 있는 다양한 편의
          시설까지 준비되어 있습니다.
        </p>
      ),
    },
  ...
  return (
    <Body>
      <HeadWrapper flex jc={"center"} pt={"50px"} pb={"50px"}>
        <HeaderNav>
          <HeaderListUl>
            {HeaderLists.map((el, key) => (
              <LI key={key} onClick={() => history.push(el.url)}>{el.list}</LI>
            ))}
          </HeaderListUl>
        </HeaderNav>
      </HeadWrapper>
      ...
      <IntroWrappeer>
        ...
        <IntroPart flex jc={"center"} mr={"620px"}>
          <h4 style={{ marginRight: "70px", paddingTop: "15px" }}>About Us</h4>
          <h1 style={{ fontSize: "40px",paddingRight:"100px" }}>
            안녕하세요.
            <br />
            국내 최대 자동차 매매단지
            <br />
        ...
        {BusinessList.map((el, key) => (
          <OurBuisinessPart key={key}>
            <Wrapper flex jc={"center"} mr={"200px"} >
              {/* <h4 style={{marginRight:"70px"}}>Our Business</h4> */}
              <h4 style={{ marginRight: "70px", paddingTop: "15px" }}>{el.title}</h4>
              <h1>{el.topic}</h1>
            </Wrapper>
            <Wrapper flex jc={"center"} mr={"200px"}>{el.content}</Wrapper>
          </OurBuisinessPart>
        ))}
      ...
      >
        <LeftPart>
          ...
          <p>
            {" "}
            -SK엔카 단지제휴를 통한 실매물 인증과 사진 촬영 제공
            <br />
            -AJ카리안서비스와 합자법인 설립하여 정확한 성능상태 점검 및 정비
            서비스 제공
            <br />
            -AJ파크 LPR 시스템으로 실시간 전시장 관리 및 고객주차장 운영
            <br />
            -현대캐피탈, 메리츠캐피탈, JB우리캐피탈 업체 입점
          </p>
        </LeftPart>
        <RightPart>
      ...
      >
        <LeftPart mr={"70px"}>
          ...
          <p>
            오토허브 입전회사 대상 그룹 게열사 금융상품을 통하여 최대 보증금 80%
            대출 가능
          </p>
        </LeftPart>
        <RightPart pr={"80px"}>
      ...
      >
        <LeftPart3 mr={"100px"} mt={"30px"}>
          ...
          <p>
            {" "}
            - 빠른 교통환경 : 수원 IC(경부/영동), 수원신갈IC(경부/용서) 인접
            <br />
            - 대규모 배후세대 : 강남권 30분 이내 및 주변 490만 배후세대의
            집객효과(흥덕, 구성, 동백, 광교 등)
            <br />- 인접지역 매매단지 연계 : 인접 매매단지(수원, 성남, 용인) 및
            타 매매단지(인천 1시간)와 유통 가능
          </p>
          <Button>
            <ButtonLabel>오시는 길 보기</ButtonLabel>
          </Button>
        </LeftPart3>
      ...
      <FootWrapper1 width={"100%"} height={"400px"} backgroundColor={"#F7F7F7"}>
        ...
        <FootInfoWrapper
          ...
          mt={"30px"}
        >
          <p style={{ fontSize: "15px", color: "#81858A" }}>
            <b style={{ marginRight: "10px" }}>사업자등록번호 </b> 454-88-00618
            <b style={{ marginLeft: "10px" }}>개인정보책임자</b>이동녕
            <br />
            <b style={{ marginRight: "10px" }}>주소</b>경기도 용인시 기흥구
            중부대로 242(영덕동 21-1)
            <br />
            <b style={{ marginRight: "10px" }}>Tel</b>031) 5182-5000Fax031)
            5182-5009{" "}
            <b style={{ marginLeft: "10px", marginRight: "5px" }}>Email</b>{" "}
            admin@nsautohub.com
            <br />
            Copyrights © AUTOHUB. All rights reserved.
          </p>
        </FootInfoWrapper>
...
const OurBuisinessPartWrapper = styled(Wrapper)``;
